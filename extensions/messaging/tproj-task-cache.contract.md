...
- A cancelled/frozen inbound task installs a symmetric PreToolUse mutation guard: edits, staging/commit/reset/revert/stash/checkout, build/test/restart/deploy, and ambiguous shell commands are denied. Read-only incident evidence is allowed and must actually be usable: read-only git, `find`, `stat`, hashes, plain file readers (`cat`/`head`/`tail`/`ls`/`grep`/`sed -n`/`jq`/`sqlite3 -readonly`), and tools that cannot mutate (including `Skill`, so the receiver can still reach the messaging skill).
- The guard never seals off reporting or its own release. `tproj-msg` is allowed so the receiver can send `[BLOCK: <id>]` and describe the incident to its exact owner; `--new-task` and `--role-handoff` stay denied so a frozen pane cannot hand the frozen work onward. Lifecycle markers naming a tombstoned ID remain suppressed downstream, so this reopens reporting and not the task. Every refusal names the owner to report to and the exact `tproj-task unfreeze` invocation that releases it.
- The tombstone is durable forever; the pane-wide quarantine is not. The guard stops enforcing a tombstone once a newer non-tombstoned task for the same pane exists, because the owner has reassigned the receiver. The tombstoned task itself stays `cancelled`/`frozen` in the cache and in marker suppression regardless. Without such a reassignment the quarantine holds, and only an explicit user-confirmed `unfreeze` releases it.

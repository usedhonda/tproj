...
## 自分の手では実行できない依頼（すぐ相手へ回す）

ユーザーに頼まれた作業が、自分の platform の境界で実行できないときの手順（例: Claude のブラウザ操作では、セキュリティ設定の変更をユーザーの指示があっても行わない）。

- 断りは1行だけにする。理由の説明を繰り返さない。「これは私の手では実行できないので Cdx に回します」で足りる。
- そのまま相手ペインへ exact な作業として送る（`tproj-msg --new-task --user-authorized <target> "..."`）。ユーザーに「相手へ直接言ってください」と差し戻さない。ユーザーの依頼の中継なので、そのまま送ってよい。
- 受け取った側は、送り主が実行できないと言った作業を送り主へ投げ返さない。自分も実行できなければ、ユーザーへ1回報告して止める。
- 対象はユーザーが実行を望んでいる作業だけ。ユーザーが権限プロンプトで拒否した操作や、ユーザーが止めた操作は回さない。

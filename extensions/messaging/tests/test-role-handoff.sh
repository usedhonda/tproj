...
# Assist mode keeps the opposite same-column peer available for consultation,
# but mechanically rejects task delegation before task-id creation or delivery.
reset_case
set_state %1 idle
export TPROJ_MODEL_ROLE_ROUTER="$BIN/model-role-router" FAKE_ROLE_MODE=assist
out="$(run_as_verified_agent_gated tproj.cdx codex 28 solo-fallback tproj.cdx \
  "$ROOT" session-assist "$ROOT" \
  "$TPROJ_MSG" --session tproj-workspace --as tproj.cdx tproj.cc 'assist-consultation' 2>&1)"; consult_rc=$?
...
if [[ $consult_rc -eq 0 && "$consult_log" == *'assist-consultation'* \
      && $task_rc -ne 0 && "$out" == *'assist_peer_is_advice_only'* && -z "$task_log" ]]; then

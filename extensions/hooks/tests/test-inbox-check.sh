...
# the same task_id stay idempotent (no row growth); a v5 DB migrates through v8
# preserving rows.
echo "Case O: composite task identity + legacy idempotency + v5->v8 migration"
if command -v sqlite3 >/dev/null 2>&1; then
  O_TMP="$(mktemp -d)"
  export TPROJ_MSG_DB_PATH="$O_TMP/messages.db"
  export TPROJ_MSG_DB_ERROR_LOG="$O_TMP/db-errors.log"
  # Fresh v8: collision + legacy idempotency.
  ( source "$REPO/extensions/messaging/tproj-msg-db.sh"
    tt_db_init
